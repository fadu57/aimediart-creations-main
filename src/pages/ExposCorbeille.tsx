...
export default function ExposCorbeille() {
  ...
  const retentionEntry = retention["expos"];
  ...
  const loadTrash = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("expos")
      .select("id, expo_name, deleted_at")
  ...
  return (
    <div className="container py-8 space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-3xl font-serif font-bold">{t("title_expos")}</h2>
      ...
      <RetentionSettingCard tableNames={["expos"]} roleId={role_id} />
