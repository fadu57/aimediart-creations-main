...
import { PromptIaPanel } from "@/components/settings/PromptIaPanel";
import { useAuthUser } from "@/hooks/useAuthUser";
import { hasFullDataAccess } from "@/lib/authUser";

export default function SettingsPromptsIa() {
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="flex items-center gap-2 text-2xl font-serif font-bold tracking-tight">
            <BrainCircuit className="h-6 w-6 text-primary" aria-hidden />
            {t("section_prompts_title")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">{t("section_prompts_desc")}</p>
        </div>
      </div>
