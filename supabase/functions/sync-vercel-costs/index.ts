/**
 * sync-vercel-costs
 * Insère le coût mensuel fixe Vercel (metadata cost_providers) dans ai_usage_events.
 * Plan Hobby → ignoré (0 $). Plan Pro → 20 $/mois.
 */
import { corsPreflightResponse, jsonResponse } from "../_shared/cors.ts";
import { getServiceRoleClient } from "../_shared/supabaseAdmin.ts";
import { requireAdminUser } from "../_shared/adminAuth.ts";
import { syncVercelMonthlyCosts } from "../_shared/vercelCostSync.ts";
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    const result = await syncVercelMonthlyCosts(admin);
    ...
    console.error("[sync-vercel-costs]", msg);
