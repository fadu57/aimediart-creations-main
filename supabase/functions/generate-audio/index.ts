...
import {
  estimateGpt4oMiniTtsCostUsd,
  GPT4O_MINI_TTS_COST_MODEL,
} from "../_shared/openAiTtsCost.ts";
...
async function insertUsageEvent(
  ...
  params: {
    ...
    costUsd: number;
    costBreakdown: ReturnType<typeof estimateGpt4oMiniTtsCostUsd>;
    status: "success" | "partial" | "error";
    uploadFailed?: boolean;
  },
): Promise<void> {
  const { payload, voiceId, latencyMs, textContent, personaVibe, fileSizeBytes, costUsd, costBreakdown, status, uploadFailed } = params;
  ...
  await supabase.from("ai_usage_events").insert({
    ...
    input_units:    costBreakdown.inputTokens,
    output_units:   costBreakdown.audioOutputTokens,
    unit_type:      "tokens",
    cost_estimated: costUsd,
    currency:       "USD",
    status,
    operation_name: payload.text_type,
    source:         "generate-audio",
    metadata: {
      text_id:             payload.text_id,
      ...(payload.text_type === "mediation" ? { artwork_id: payload.text_id } : {}),
      lang:                payload.lang,
      prompt_style_id:     payload.prompt_style_id,
      gender:              payload.gender,
      voice_id:            voiceId,
      ...
      instruction_chars:   personaVibe.length,
      file_size_bytes:     fileSizeBytes,
      audio_duration_sec:  costBreakdown.audioDurationSec,
      cost_model:          GPT4O_MINI_TTS_COST_MODEL,
      cost_input_usd:      costBreakdown.inputCostUsd,
      cost_audio_usd:      costBreakdown.audioCostUsd,
      upload_failed:       uploadFailed ?? false,
    },
  });
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { text_id, text_type, lang, prompt_style_id, gender } = payload;
    ...
    const personaVibe = (style.persona_vibe ?? "").trim();
    ...
    const costBreakdown = estimateGpt4oMiniTtsCostUsd({
      textChars: textContent.length,
      instructionChars: personaVibe.length,
      audioFileSizeBytes: fileSizeBytes,
    });
    const costUsd = costBreakdown.totalUsd;
    ...
    await supabase.from("audio_files").update({
      status:          "ready",
      file_size_bytes: fileSizeBytes,
      input_tokens:    costBreakdown.inputTokens,
      cost_usd:        costUsd,
      error_message:   null,
      updated_at:      new Date().toISOString(),
    })
