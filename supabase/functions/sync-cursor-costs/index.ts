/**
 * sync-cursor-costs
 * Insère le coût mensuel fixe Cursor (metadata cost_providers) dans ai_usage_events.
 * Appel cron (pg_cron + service role) ou manuel admin.
 */
import { corsPreflightResponse, jsonResponse } from "../_shared/cors.ts";
import { getServiceRoleClient } from "../_shared/supabaseAdmin.ts";
import { requireAdminUser } from "../_shared/adminAuth.ts";
import { syncCursorMonthlyCosts } from "../_shared/cursorCostSync.ts";
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    const result = await syncCursorMonthlyCosts(admin);
    ...
    console.error("[sync-cursor-costs]", msg);
