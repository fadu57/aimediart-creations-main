-- Souscription self-service organisation + remises commerciales (snapshot)
BEGIN;

DO $$ BEGIN
  CREATE TYPE public.commercial_kind AS ENUM (
    'standard',
    'partner_showcase',
    'sponsoring',
    'internal_test'
...
ALTER TABLE public.agencies
  ADD COLUMN IF NOT EXISTS commercial_kind public.commercial_kind NOT NULL DEFAULT 'standard',
...
ALTER TABLE public.agencies
  DROP CONSTRAINT IF EXISTS agencies_discount_percent_chk;

ALTER TABLE public.agencies
  ADD CONSTRAINT agencies_discount_percent_chk
  CHECK (discount_percent >= 0 AND discount_percent <= 100);

ALTER TABLE public.agencies
  DROP CONSTRAINT IF EXISTS agencies_discount_amount_eur_chk;

ALTER TABLE public.agencies
  ADD CONSTRAINT agencies_discount_amount_eur_chk
  CHECK (discount_amount_eur >= 0);

COMMENT ON COLUMN public.agencies.commercial_kind IS
  'Profil commercial preset (vitrine partenaire, sponsoring, test interne).';

ALTER TABLE public.organisation_subscriptions
  ADD COLUMN IF NOT EXISTS list_price_eur numeric(10,2),
  ADD COLUMN IF NOT EXISTS discount_percent numeric(5,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS discount_amount_eur numeric(10,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS net_price_eur numeric(10,2),
  ADD COLUMN IF NOT EXISTS commercial_kind public.commercial_kind NOT NULL DEFAULT 'standard',
  ADD COLUMN IF NOT EXISTS sponsor_valid_until timestamptz;

ALTER TABLE public.organisation_subscriptions
  DROP CONSTRAINT IF EXISTS organisation_subscriptions_discount_percent_chk;

ALTER TABLE public.organisation_subscriptions
  ADD CONSTRAINT organisation_subscriptions_discount_percent_chk
  CHECK (discount_percent >= 0 AND discount_percent <= 100);

ALTER TABLE public.organisation_subscriptions
  DROP CONSTRAINT IF EXISTS organisation_subscriptions_discount_amount_eur_chk;

ALTER TABLE public.organisation_subscriptions
  ADD CONSTRAINT organisation_subscriptions_discount_amount_eur_chk
  CHECK (discount_amount_eur >= 0);

CREATE OR REPLACE FUNCTION public.compute_subscription_net_price(
  p_list_price numeric,
  p_discount_percent numeric,
  p_discount_amount_eur numeric
)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
...
COMMENT ON FUNCTION public.subscribe_organisation_plan(public.pricing_plan_code, public.billing_cycle, uuid) IS
  'Souscription self-service : admin org (role 4) ou admin global. Applique les remises preset agencies + snapshot.';
