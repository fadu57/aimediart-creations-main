-- Migration 72 : présence visiteur audio + bannissement admin (expo intérieure)
-- Heartbeat via Edge Function visitor-audio-session (service role)
-- Lecture admin : rls_is_global_admin ; Realtime sur banned_at pour coupure instantanée
...
-- Realtime (coupe audio instantanée côté visiteur)
