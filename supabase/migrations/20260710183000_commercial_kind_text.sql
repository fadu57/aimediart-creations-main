-- Profils commerciaux libres (texte) au lieu de l'enum figé.
BEGIN;

ALTER TABLE public.agencies
  ALTER COLUMN commercial_kind DROP DEFAULT;

ALTER TABLE public.agencies
  ALTER COLUMN commercial_kind TYPE text USING commercial_kind::text;

ALTER TABLE public.agencies
  ALTER COLUMN commercial_kind SET DEFAULT 'standard';
...
COMMENT ON COLUMN public.agencies.commercial_kind IS
  'Profil commercial (preset standard/partner_showcase/sponsoring/internal_test ou libellé personnalisé).';

-- subscribe_organisation_plan : variable commercial_kind en text
...
DROP TYPE IF EXISTS public.commercial_kind;
