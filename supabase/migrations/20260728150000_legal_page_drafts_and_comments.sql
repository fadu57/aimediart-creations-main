-- Versions provisoires des pages légales + commentaires de relecture (juriste / admin 1).
-- Idempotent : crée la table si absente, puis ajoute draft/published + commentaires.
-- À exécuter dans Supabase → SQL Editor → Run (tout le fichier).
...
COMMENT ON COLUMN public.legal_page_contents.status IS
  'draft = version provisoire avant validation ; published = version publique.';
...
COMMENT ON TABLE public.legal_page_comments IS
  'Annotations de relecture sur une version provisoire de page légale (juriste / admin 1 uniquement).';
