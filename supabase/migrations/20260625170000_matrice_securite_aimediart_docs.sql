-- 20260625170000_matrice_securite_aimediart_docs.sql
-- Étend la contrainte CHECK matrice_securite_ressource_chk pour autoriser les
-- 3 sections de documents AIMEDIArt (accordéons de la page /settings) dans la
-- matrice des droits. Clés alignées avec NAV_MATRIX_SUBPAGE_DEFS (navigationMatrix.ts).
...
ALTER TABLE public.matrice_securite ADD CONSTRAINT matrice_securite_ressource_chk CHECK (
  ressource = ANY (
    ARRAY[
      ...
      -- Documents AIMEDIArt (accordéons /settings)
