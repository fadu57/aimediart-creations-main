-- Assouplir l'autorisation de création de lien : alignement visitor_id / visitor_client_id / auth.
  ...
  IF auth.uid() IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      WHERE vf.visitor_id = v_visitor_id
        AND (
          lower(vf.visitor_id) = lower(auth.uid()::text)
          OR EXISTS (
            SELECT 1
            FROM public.visitors v
            WHERE v.auth_user_id = auth.uid()
              AND (
                v.visitor_client_id = vf.visitor_id
                OR v.id::text = vf.visitor_id
                OR lower(v.auth_user_id::text) = lower(vf.visitor_id)
              )
          )
        )
      LIMIT 1
